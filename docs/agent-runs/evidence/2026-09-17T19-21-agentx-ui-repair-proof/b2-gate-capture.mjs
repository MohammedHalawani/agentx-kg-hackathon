/**
 * Agent D (B2 Gate) — independent verification @ 1440×900
...
// Schema 2D/3D theme lifecycle via UI ThemeToggle
...
// Graph lens empty state (expected while /graph 500)
...
writeFileSync(join(OUT, 'measurements-b2-gate.json'), JSON.stringify(report, null, 2));
await browser.close();
console.log('B2 gate issues:', report.issues.length, report.issues);
...
console.log('written', join(OUT, 'measurements-b2-gate.json'));
