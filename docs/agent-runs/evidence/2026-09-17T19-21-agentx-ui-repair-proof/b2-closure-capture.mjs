/**
 * Closure Reviewer 5B — B2 revalidation in FINAL integrated app (post-B4)
 * Candidate: index-DVhlRZ8U.css / index-DE557tU5.js @ http://127.0.0.1:8000
 */
...
const SHOT = (name) => join(OUT, `${name}-closure.png`);
...
report.buildMatch = servedCss.includes(CANDIDATE_CSS) && servedJs.includes(CANDIDATE_JS);
if (!report.buildMatch) {
  report.issues.push({
    id: 'BUILD',
    severity: 'BLOCKER',
...
if (report.endpoint.graph?.status === 500) {
  report.verdicts.GRAPH_ENDPOINT = 'BLOCKED';
}
...
writeFileSync(join(OUT, 'measurements-b2-closure.json'), JSON.stringify(report, null, 2));
await browser.close();
console.log('B2 closure issues:', report.issues.length, report.issues);
...
console.log('written', join(OUT, 'measurements-b2-closure.json'));
