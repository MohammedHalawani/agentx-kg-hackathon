/**
 * Agent A (B2 Audit) — theme lifecycle + graph chrome verification @ 1440×900
...
// B1 hover sentinels H01-H05 (regression check)
...
// Explore — Schema lens (live /schema data) for E02/E04 chrome
...
for (const theme of ['light', 'dark']) {
  ...
  const e01Lens = await measure(page, 'button .relative.z-10');
  const e01Pill = await measure(page, 'button .absolute.inset-0.rounded-md.bg-primary');
  if (e01Lens && e01Pill) {
    const ratio = await contrastRatio(page, e01Lens.color, e01Pill.backgroundColor);
    report.measurements.push({ theme, view: 'E01-explore-lens', ratio, lens: 'schema', ...e01Lens, pillBg: e01Pill.backgroundColor });
    if (ratio !== null && ratio < 4.5) report.issues.push({ id: 'E01', theme, ratio });
  }

  const layoutBtn = page.locator('button').filter({ hasText: /Force|Tree/i }).first();
  if (await layoutBtn.count()) {
    ...
    await layoutBtn.hover();
    await page.waitForTimeout(200);
    const layoutHover = await measure(page, 'button:hover');
    report.measurements.push({ theme, view: 'E02-layout-hover', pointerHover: true, ...layoutHover });
  }

  const renderer2d = page.locator('button').filter({ hasText: /2D/i }).first();
  if (await renderer2d.count()) {
    const rendererSelected = await measure(page, 'button.bg-primary.text-primary-foreground');
    if (rendererSelected) {
      const ratio = await contrastRatio(page, rendererSelected.color, rendererSelected.backgroundColor);
      report.measurements.push({ theme, view: 'E02-renderer-selected', ratio, fixture: 'schema-live', ...rendererSelected });
      if (ratio !== null && ratio < 4.5) report.issues.push({ id: 'E02', theme, ratio, control: 'renderer' });
    }
  }

  const legend = await measure(page, '.absolute.left-3.top-3');
  if (legend) {
    report.measurements.push({ theme, view: 'E04-legend-overlay', ...legend });
  }

  const shot2d = join(OUT, `b2-schema-2d-${theme}.png`);
  ...
  if (await renderer3d.count()) {
    ...
    const canvasBg = await page.evaluate(() => {
      ...
      return { wrapBg, canvasCount: document.querySelectorAll('canvas').length };
    });
    report.measurements.push({ theme, view: 'E03-3d-wrap', fixture: 'schema-live', ...canvasBg });
    const shot3d = join(OUT, `b2-schema-3d-${theme}.png`);
...
// Theme toggle without remount: light → dark → light on mounted Schema view
...
const toggleShot = join(OUT, 'b2-theme-toggle-no-remount.png');
await page.screenshot({ path: toggleShot, fullPage: false });
report.captures.push(toggleShot);

// Graph lens empty state (blocked)
await page.locator('button').filter({ hasText: /^Graph/i }).click();
await page.waitForTimeout(2500);
const graphEmpty = await page.evaluate(() => document.body.innerText.includes('No graph data') || document.body.innerText.includes('no graph'));
report.graphLensEmpty = graphEmpty;
const graphShot = join(OUT, 'b2-graph-empty-light.png');
...
for (const id of ['H01', 'H02', 'H03', 'H04', 'H05', 'E01', 'E02', 'E03', 'E04']) {
  const idIssues = report.issues.filter((i) => i.id === id);
  if (id === 'E03' && graphBlocked) {
    report.verdicts[id] = idIssues.length ? 'BLOCKED (issues in schema-3d proxy)' : 'BLOCKED (no /graph live 3D)';
  } else if ((id === 'E02' || id === 'E04') && graphBlocked) {
    report.verdicts[id] = idIssues.length ? 'REPRODUCED (schema proxy)' : graphBlocked ? 'PARTIAL (schema proxy PASS)' : 'PASS';
  } else {
    report.verdicts[id] = idIssues.length ? 'REPRODUCED' : 'NOT REPRODUCED';
  }
}

writeFileSync(join(OUT, 'measurements-b2-audit.json'), JSON.stringify(report, null, 2));
await browser.close();
console.log('B2 audit issues:', report.issues.length, report.issues);
console.log('Verdicts:', report.verdicts);
console.log('written', join(OUT, 'measurements-b2-audit.json'));
