...
import { AlertTriangle, CheckCircle2, Inbox, Repeat, TrendingUp } from 'lucide-react'
...
export function DecisionsView() {
  ...
  if (loading || !data) {
    return (
      <div className="grid gap-4 overflow-y-auto p-6 md:grid-cols-2 xl:grid-cols-4">
  ...
  const { coverage, by_category, by_action, writebacks, queue } = data
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        ...
        <Card title="Closed-loop learning" subtitle="Resolutions the pipeline wrote back, against the seeded history">
          <StackedBar
            segments={[
              {
                key: 'agent',
              ...
              { key: 'seeded', label: 'Seeded history', value: writebacks.seeded, colorClass: 'bg-chart-aqua' },
            ]}
          />
          <p className="mt-3 text-xs text-muted">
            Each accepted recommendation becomes a new precedent, retrievable by the next case's vector search.
          </p>
        </Card>
        ...
        <Card title="Success rate by action" subtitle="The action vocabulary, ranked by how well it has held up">
            ...
            <tbody>
              {queue.map((c) => (
                <tr key={c.failure_id} className="border-t border-hairline">
                  <td className="py-1.5 pr-3 font-mono text-[11px] text-muted">{c.failure_id}</td>
                  <td className="py-1.5 pr-3 text-ink">{c.shipment_id ?? '—'}</td>
                  <td className="py-1.5 pr-3 text-ink">{label(c.category)}</td>
